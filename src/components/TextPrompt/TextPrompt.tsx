...
export default function TextPrompt({ status_gen, status_erro }: Props) {
  ...
  const SendData = () => {
    if (inputText != "") {
      ...
      axios
        .post(`${import.meta.env.VITE_URL_GENERATE}`, { prompt: inputText })
        .then((response) => {
          if (status_erro) {
            setOpenError(true);
          }
  ...
  return (
    <div className="flex justify-center">
      <div className="form-control w-3/4 md:w-4/5 ">
        <div className="input-group rounded rounded-full flex justify-center">
          <input
            type="text"
          ...
          <Tooltip className="text" content={t("tool-tip-generate")}>
            <button
              className={`${
                !status_gen
                  ? "btn btn-square h-14 w-14 md:h-20 md:w-20 btn-search relative mobile-top right-3 md:right-5 hover:bg-blue-500 btn-disable"
                  : "btn btn-square h-14 w-14 md:h-20 md:w-20 btn-search relative mobile-top right-3 md:right-5 hover:bg-blue-500 hover:right-3 hover:transition-all hover:duration-400 transition-all duration-300"
              }`}
              onClick={SendData}
              disabled={!status_gen}
            >
              <IconArrowRight
                className="relative right-0 bounce2 text-white"
                width={"30"}
                height={"30"}
              />
            </button>
      ...
      <Alert
        ...
        onClose={() => {
          axios
            .post(`${import.meta.env.VITE_URL_STATUS_ERR}`, {
              status_erro: false,
            })
            .then(() => {
              setInputText("");
